...
class LoRa_IF:    
    ...
    def join_sequence(self, dev_eui, app_key):
        ...
        print('Joined')
        lora.nvram_save()
        lat, lon, gps_attempts = GPS_module().get_current_pos(10)
        s = self.set_socket_configuration()
        self.send_packet(s, lat, lon, gps_attempts)

        afdsdCtrl2().go_to_sleep(20000)#3600000) #3600000 = 1 Hour

    def send_packet(self, s, lat, lon, gps_attempts) :
	    #Serverside will receive a 12 byte payload on the form
        payload = bytes(struct.pack("fff", lat, lon, gps_attempts)) 
...
class afdsdCtrl2:
    ...
    def deepsleep_wakeup_sequence(self):
        ...
        lora_if.send_packet(s, lat, lon, gps_attempts)
